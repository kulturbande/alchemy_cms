...
module Alchemy
  RSpec.describe WildcardUrlType do
    ...
    describe "#assert_valid_value" do
      ...
      it "raises for a Hash with an unknown format matcher in params" do
        expect {
          type.assert_valid_value({"pattern" => ":foo", "params" => {"foo" => "nonexistent"}})
        }.to raise_error(
          ArgumentError, /Unknown format matcher "nonexistent".*Available matchers:/
        )
      end

      it "accepts a Hash with a Regexp constraint in params" do
        expect {
          type.assert_valid_value({"pattern" => ":sku", "params" => {"sku" => /\A[A-Z]+\z/}})
        }.not_to raise_error
      end

      it "accepts a Hash with a valid named constraint in params" do
        expect {
          type.assert_valid_value({"pattern" => ":id", "params" => {"id" => "integer"}})
        }.not_to raise_error
      end
    end
    ...
    describe WildcardUrlType::Value do
      ...
      describe "#constraint_pattern" do
        it "returns a catch-all pattern when no constraint is defined" do
          value = described_class.new(pattern: ":slug")
          expect(value.constraint_pattern("slug")).to eq("[^/]+")
        end

        it "returns the format matcher pattern for a named constraint" do
          value = described_class.new(pattern: ":id", params: {"id" => "integer"})
          expect(value.constraint_pattern("id")).to eq("\\d+")
        end

        it "strips anchors from a Regexp constraint" do
          value = described_class.new(pattern: ":sku", params: {"sku" => /\A[A-Z]{2}-\d{4}\z/})
          expect(value.constraint_pattern("sku")).to eq("[A-Z]{2}-\\d{4}")
        end

        it "returns the format matcher pattern for a string constraint" do
          value = described_class.new(pattern: ":slug", params: {"slug" => "string"})
          expect(value.constraint_pattern("slug")).to eq("[^/]+")
        end

        it "returns the correct pattern for each param in a multi-segment pattern" do
          value = described_class.new(pattern: ":year/:slug", params: {"year" => "integer", "slug" => "string"})
          expect(value.constraint_pattern("year")).to eq("\\d+")
          expect(value.constraint_pattern("slug")).to eq("[^/]+")
        end

        it "preserves ^ inside character classes when stripping anchors" do
          value = described_class.new(pattern: ":code", params: {"code" => /\A[^\/]+\z/})
          expect(value.constraint_pattern("code")).to eq("[^/]+")
        end
      end
